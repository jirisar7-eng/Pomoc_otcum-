...
import { Scale, Search, ExternalLink, Bookmark, HelpCircle, FileText, CheckCircle } from 'lucide-react';
...
const INITIAL_RULINGS: Ruling[] = [
  ...
  },
  {
    ...
    categories: ['střídavá péče', 'nejlepší zájem dítěte', 'kojenec', 'věk dítěte', 'přespávání'],
  ...
  },
  {
    ...
    categories: ['střídavá péče', 'věk dítěte', 'přespávání'],
  ...
  },
  {
    ...
    categories: ['nejlepší zájem dítěte', 'věk dítěte'],
  ...
  },
  {
    ...
    categories: ['výživné', 'střídavá péče'],
  ...
  },
  {
    ...
    date: '2013-09-30',
    court: 'Ústavní soud ČR',
    summary: 'Zásadním zájmem dětí je vyrůstat společně. Rozdělení sourozenců (např. syn k otci, dcera k matce) is možné pouze za výjimečných a prokazatelných okolností. Výchovný režim (včetně střídavé péče) by měl být pro sourozence zpravidla nastaven jednotně.',
  ...
  },
  {
    ...
    categories: ['předběžné opatření', 'nejlepší zájem dítěte', 'přespávání'],
...
export default function JudikaturaSection() {
  ...
  // Synced Saved Judgments with User Portal Tab
  const [savedJudgments, setSavedJudgments] = useState<any[]>(() => {
    try {
      const raw = localStorage.getItem('sh_portal_saved_judgments');
  ...
  const filterOptions = [
    { label: 'Všechna rozhodnutí', value: null },
    { label: 'Věk dítěte', value: 'věk dítěte' },
    { label: 'Přespávání', value: 'přespávání' },
    { label: 'Střídavá péče', value: 'střídavá péče' },
    { label: 'Společná péče', value: 'společná péče' },
    { label: 'Sourozenci', value: 'sourozenci' },
    { label: 'Kojenec', value: 'kojenec' },
    { label: 'OSPOD / Kolizní opatrovník', value: 'OSPOD' },
    { label: 'Výživné / Alimenty', value: 'výživné' }
  ];
  ...
  const toggleBookmark = (ruling: Ruling) => {
    setSavedJudgments(prev => {
      const exists = prev.some(j => j.id === ruling.id);
      if (exists) {
        return prev.filter(j => j.id !== ruling.id);
      } else {
        return [...prev, {
          id: ruling.id,
          courtName: ruling.court,
  ...
  const isBookmarked = (id: string) => {
    return savedJudgments.some(j => j.id === id);
  };

  return (
    <div className="space-y-8" id="judikatura-section-container">
      {/* Page Header */}
      <div className="bg-white rounded-3xl border border-slate-200/60 p-6 md:p-8 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] uppercase font-bold text-teal-600 tracking-wider font-mono">Právní opora</span>
            <h2 className="text-xl md:text-2xl font-bold text-slate-800 font-display">Klíčová judikatura a nálezy</h2>
          </div>
        </div>
        <p className="text-slate-500 text-xs leading-relaxed max-w-3xl mt-3">
          Rozhodnutí Ústavního soudu České republiky jsou pro obecné opatrovnické soudy právně závazná. Pokud protistrana, soud nebo OSPOD tvrdí skutečnosti v rozporu s těmito judikáty, můžete se na ně ve svých vyjádřeních odvolat.
        </p>
      </div>

      {/* Control Panel: Search & Filters */}
      <div className="bg-white p-5 rounded-3xl border border-slate-200/60 flex flex-col gap-4 shadow-xs">
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Hledat spisovou značku, právní názor nebo slovo v textu..."
            className="w-full pl-9 pr-4 py-2 bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-teal-500 rounded-xl text-xs outline-none transition-all"
          />
        </div>

        {/* Czech Filter Tags bar */}
      ...
      <div className="space-y-6" id="judikatura-feed-list">
        {filteredRulings.length === 0 ? (
          <div className="bg-white p-12 rounded-3xl border border-slate-200/60 text-center text-slate-400 text-xs italic">
            Nebyly nalezeny žádné judikáty odpovídající tomuto filtru.
          </div>
        ) : (
          filteredRulings.map(ruling => (
            <div 
              key={ruling.id} 
              className="bg-white rounded-3xl border border-slate-200/60 hover:border-slate-300 p-6 shadow-xs flex flex-col md:flex-row gap-6 transition-all"
            ...
            >
              ...
              <div className="md:w-56 shrink-0 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-100 pb-4 md:pb-0 md:pr-6">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5 font-mono">Vydáno: {new Date(ruling.date).toLocaleDateString('cs-CZ')}</p>
                </div>

                <div className="mt-4 flex flex-wrap gap-1">
                  {ruling.categories.map(c => (
                    <span key={c} className="text-[9px] font-mono bg-slate-50 text-slate-500 px-1.5 py-0.5 rounded border border-slate-100 font-semibold uppercase">
                      {c}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex-grow space-y-4">
                <div className="flex justify-between items-start gap-4">
                  <h4 className="text-sm font-bold text-slate-800 font-display leading-snug">{ruling.title}</h4>
                  <button 
                    onClick={() => toggleBookmark(ruling)} 
                    className="p-1.5 text-slate-400 hover:text-teal-600 rounded-lg hover:bg-slate-50 transition-all cursor-pointer shrink-0"
                    title={isBookmarked(ruling.id) ? "Odebrat z uložených" : "Uložit do osobního portálu"}
                  >
                    <Bookmark className={`w-4 h-4 ${isBookmarked(ruling.id) ? 'fill-teal-600 text-teal-600' : ''}`} />
                  </button>
                </div>

                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-150 relative">
                  <span className="absolute -top-2.5 left-4 px-2 bg-white text-[9px] font-bold text-slate-400 uppercase tracking-wider border border-slate-200 rounded-md font-mono">Klíčový právní názor ÚS</span>
                  <p className="text-xs text-slate-650 leading-relaxed italic mt-1 font-sans">
                    "{ruling.summary}"
                  </p>
                </div>
                ...
                <div className="space-y-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1 font-mono">
                    ...
                    Jak to prakticky využít ve vašem podání:
                  </span>
                  <p className="text-xs text-slate-600 leading-relaxed pl-4 border-l-2 border-teal-500/30">
                ...
                <div className="pt-2 flex justify-end">
                  <a 
                    ...
                    className="flex items-center gap-1 text-[11px] text-teal-700 hover:text-slate-900 font-bold transition-all hover:underline"
                  >
      ...
      {/* Quick advice banner */}
      <div className="bg-slate-50 border border-slate-200 p-6 rounded-3xl flex flex-col md:flex-row items-center gap-4 justify-between">
        <div className="space-y-1 text-center md:text-left">
          <h4 className="font-bold text-slate-800 text-sm font-display">Potřebujete vyhledat doplňující judikaturu?</h4>
          <p className="text-slate-500 text-xs">
            Můžete se zeptat našeho AI Průvodce, který má přehled o všech klíčových rozhodnutích opatrovnických soudů.
          </p>
        </div>
        <button 
          onClick={() => {
          ...
          className="bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs px-4 py-2.5 rounded-xl shrink-0 transition-colors cursor-pointer"
        >
          Zeptat se AI průvodce
        </button>
      </div>
